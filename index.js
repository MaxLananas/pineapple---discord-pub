...
if (content === '!pubstats') {
  ...
  const sorted = Array.from(pubStats.entries()).sort((a, b) => b[1] - a[1]).slice(0, 10); // Top 10
  const leaderboard = sorted.map(([userId, count], index) => `**#${index + 1}** - <@${userId}> : ${count} pub(s)`).join('\n');
